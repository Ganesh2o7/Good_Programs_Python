...
def load_dataset(file_path, test_size=0.3):
    """Load dataset, generate pseudo-labels using k-means, and split into train/test sets."""
...
def plot_data(X_train, Y_train, X_test, Y_test, dataset_name):
    ...
    plt.title(f"Training Data: {dataset_name} (Pseudo-Labels)")
    plt.show()
    ...
    plt.xlabel("Feature 1")
    plt.ylabel("Feature 2")
    plt.title(f"Testing Data: {dataset_name} (Pseudo-Labels)")
    plt.show()
